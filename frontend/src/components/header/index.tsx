...
const Header = (props: { sidebarOpen: string | boolean | undefined; setSidebarOpen: (arg0: boolean) => void }) => {
 ...
 // useEffect(() => {
 // 	const header = document.getElementById('myBody')
 ...
 // 	console.log('header', header)
 // 	if (header) {
 // 		setSticky(header.getBoundingClientRect().top <= 0)
 // 	}
 // }

 return (
  <header id='myHeader' style={{ height: '70px', position: isSticky ? 'fixed' : 'relative' }} className={`z-9 flex w-full  dark:drop-shadow-none ${isSticky ? 'bg-white shadow-md' : 'bg-white shadow-md'}`}>
   <div style={{ height: '70px' }} className='flex flex-grow items-center justify-between lg:justify-end px-4  md:px-6 '>
    <div className='flex items-center gap-2 sm:gap-4 lg:hidden'>
     ...
     <button
      ...
      className='z-1024 block rounded-sm border border-stroke bg-white p-1.5 shadow-sm dark:border-strokedark dark:bg-boxdark lg:hidden'>
